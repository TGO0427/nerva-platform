...
import {
  ...
  CycleCount as CycleCountEntity,
  CycleCountLine as CycleCountLineEntity,
} from "./cycle-count.repository";
import {
  PutawayRepository,
  PutawayTaskDetail,
  PutawayFilters,
} from "./putaway.repository";
...
export class InventoryService {
  ...
  async createAdjustment(data: {
    ...
    const adjustmentNo = await this.repository.generateAdjustmentNo(
      data.tenantId,
    );
  ...
  async importAdjustments(
    ...
    const warehouseMap = await this.masterDataService.resolveWarehouseNames(
      tenantId,
      siteId,
      uniqueWarehouseNames,
    );
    const unknownWarehouses = uniqueWarehouseNames.filter(
      (n) => !warehouseMap.has(n.toLowerCase()),
    );
    if (unknownWarehouses.length > 0) {
      throw new BadRequestException(
        `Unknown warehouse names: ${unknownWarehouses.join(", ")}`,
      );
    }
    ...
    for (const [warehouseId, codes] of warehouseBinCodes) {
      const binMap = await this.masterDataService.resolveBinCodes(
        tenantId,
        warehouseId,
        [...codes],
      );
    ...
    if (unknownBins.length > 0) {
      throw new BadRequestException(
        `Unknown bin codes: ${unknownBins.join(", ")}`,
      );
    }
  ...
  async addCycleCountLinesFromWarehouse(
    ...
    const stock = await this.stockLedger.getStockInWarehouse(
      tenantId,
      cc.warehouseId,
    );
    if (stock.length === 0) {
  ...
  async completeCycleCount(id: string): Promise<CycleCountEntity> {
    ...
    if (counted < total) {
      throw new BadRequestException(
        `All lines must be counted (${counted}/${total} done)`,
      );
    }
    const updated = await this.cycleCountRepo.updateStatus(
      id,
      "PENDING_APPROVAL",
    );
    return updated!;
  ...
  async completePutawayTask(
    ...
    if (bin.binType !== "STORAGE" && bin.binType !== "PICKING") {
      throw new BadRequestException(
        "Target bin must be a STORAGE or PICKING bin",
      );
    }
