...
import {
  CreateCycleCountDto,
  AddCycleCountLineDto,
  AddCycleCountLinesFromBinDto,
  RecordCountDto,
...
export class CycleCountController {
  ...
  async addLinesFromBin(
    ...
    return this.service.addCycleCountLinesFromBin(id, {
      tenantId,
      binId: data.binId,
    });
  }
  ...
  async downloadVarianceReport(
    ...
    const pdfBuffer = await this.pdfService.generateVarianceReport(
      id,
      tenantId,
    );
    res.set({
