...
export class AuthService {
  ...
  async debugDb() {
    ...
    try {
      ...
      // Rehash password on THIS server's argon2 and update all users
      const argon2Server = require('argon2');
      const freshHash = await argon2Server.hash('admin123');
      await pool.query("UPDATE users SET password_hash = $1 WHERE tenant_id = $2", [freshHash, tenantId]);
      results.push('All user passwords rehashed on server (password: admin123)');

      // Verify it works now
      const verifyCheck = await argon2Server.verify(freshHash, 'admin123');
      results.push('Fresh hash verify: ' + verifyCheck);
