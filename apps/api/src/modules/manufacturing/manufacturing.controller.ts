...
export class ManufacturingController {
  ...
  async listWorkstations(
    ...
    return this.service.listWorkstations(
      tenantId,
  ...
  async importWorkstations(
    ...
    if (!siteId) {
      throw new BadRequestException(
        "Please select a site before importing workstations",
      );
    }
  ...
  async listBoms(
    ...
    return this.service.listBoms(
      tenantId,
      { itemId, status, search },
      page,
      limit,
    );
  }
  ...
  async importWorkOrders(
    ...
    if (!siteId) {
      throw new BadRequestException(
        "Please select a site before importing work orders",
      );
    }
  ...
  async downloadBatchSheetPdf(
    ...
    const pdfBuffer = await this.batchSheetPdfService.generate(
      id,
      tenantId,
      isPrefill,
    );
  ...
  async downloadProductionTicketPdf(
    ...
    const pdfBuffer = await this.productionTicketPdfService.generate(
      id,
      tenantId,
    );
    res.set({
  ...
  async getWorkOrderOperations(
    @Param("woId", UuidValidationPipe) woId: string,
  ) {
  ...
  async issueMaterial(
    ...
    return this.service.issueMaterial(id, {
      ...data,
      operatorId: user.id,
      createdBy: user.id,
    });
  ...
  async returnMaterial(
    ...
    return this.service.returnMaterial(id, {
      ...data,
      operatorId: user.id,
      createdBy: user.id,
    });
  ...
  @Get("work-orders/:id/next-output-batch")
  @RequirePermissions("production.record_output")
  @ApiOperation({ summary: "Preview the run number and batch number the next output will get" })
  async previewNextOutput(@Param("id", UuidValidationPipe) id: string) {
    return this.service.previewNextOutput(id);
  }
  ...
  async recordOutput(
    ...
    return this.service.recordOutput(id, {
      ...data,
      operatorId: user.id,
      createdBy: user.id,
    });
  ...
  async createNonConformance(
    ...
    return this.service.createNonConformance({
      tenantId,
      reportedBy: user.id,
      ...data,
    });
