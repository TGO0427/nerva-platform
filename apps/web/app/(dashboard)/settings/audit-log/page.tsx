...
function JsonDiff({ before, after }: { before: Record<string, unknown> | null; after: Record<string, unknown> | null }) {
  if (!before && !after) {
    return <p className="text-sm text-text-muted dark:text-text-dark-muted">No data recorded</p>;
  }
  ...
  if (changes.length === 0 && before && after) {
    return <p className="text-sm text-text-muted dark:text-text-dark-muted">No field changes detected</p>;
  }
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b">
              <th className="text-left py-1 pr-4 font-medium text-text-secondary dark:text-text-dark-secondary">Field</th>
              <th className="text-left py-1 pr-4 font-medium text-text-secondary dark:text-text-dark-secondary">Before</th>
              <th className="text-left py-1 font-medium text-text-secondary dark:text-text-dark-secondary">After</th>
            </tr>
          </thead>
          <tbody>
            {changes.map((c) => (
              <tr key={c.key} className="border-b border-surface-border dark:border-surface-dark-border">
                <td className="py-1 pr-4 font-medium text-text-secondary dark:text-text-dark-secondary">{c.key}</td>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-primary dark:text-text-dark-primary">Audit Log</h1>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <svg className="mx-auto h-12 w-12 text-text-muted dark:text-text-dark-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <h3 className="mt-2 text-sm font-medium text-text-primary dark:text-text-dark-primary">No audit log entries</h3>
              ...
              <table className="min-w-full divide-y divide-surface-border dark:divide-surface-dark-border">
                <thead className="bg-surface-secondary dark:bg-surface-dark-secondary">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-text-muted dark:text-text-dark-muted uppercase tracking-wider">
                      Timestamp
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-text-muted dark:text-text-dark-muted uppercase tracking-wider">
                      User
                  ...
                  {entries.map((entry) => (
                    <Fragment key={entry.id}>
                      <tr
                        ...
                        className="hover:bg-surface-secondary dark:hover:bg-surface-dark-secondary cursor-pointer transition-colors"
                      >
                        <td className="px-3 py-4 text-text-muted dark:text-text-dark-muted">
                          <svg
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-xs text-text-secondary dark:text-text-dark-secondary">
                          {formatDateTime(entry.createdAt)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {entry.actorName || <span className="text-text-muted dark:text-text-dark-muted">System</span>}
                        </td>
                      ...
                      {expandedId === entry.id && (
                        <tr key={`${entry.id}-detail`}>
                          <td colSpan={6} className="px-6 py-4 bg-surface-secondary dark:bg-surface-dark-secondary">
                            <div className="flex items-center gap-6 mb-3 text-xs text-text-muted dark:text-text-dark-muted">
