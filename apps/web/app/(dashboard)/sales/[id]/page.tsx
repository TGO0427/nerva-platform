...
export default function SalesOrderDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    if (confirmed) {
      try {
        await deleteSalesOrder.mutateAsync(orderId);
  ...
  const handleReopen = async () => {
    const message = order?.status === 'CANCELLED'
      ? 'Reopen this cancelled order? It will return to DRAFT status.'
      : 'Reopen this delivered order? It will return to SHIPPED status.';
    const confirmed = await confirm({
      title: 'Reopen Order',
      message,
    ...
    if (confirmed) {
      try {
        ...
        addToast('Order reopened', 'success');
      } catch (error) {
        console.error('Failed to reopen order:', error);
        addToast('Failed to reopen order', 'error');
      }
    }
  };
