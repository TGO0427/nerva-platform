...
export default function ShipmentDetailPage() {
  ...
  const reopenShipment = useReopenShipment();
  ...
  const handleReopen = async () => {
    const confirmed = await confirm({
      ...
      message: 'Reopen this delivered shipment? It will return to SHIPPED status.',
      confirmLabel: 'Reopen',
    });
    if (!confirmed) return;
    try {
      await reopenShipment.mutateAsync(shipmentId);
      addToast('Shipment reopened', 'success');
    } catch (error) {
      console.error('Failed to reopen shipment:', error);
      addToast('Failed to reopen shipment', 'error');
    }
  };

  const handlePrintPackingSlip = async () => {
    try {
      const response = await api.get(`/fulfilment/shipments/${shipmentId}/packing-slip`, {
        responseType: 'blob',
      });
      const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `packing-slip-${shipment?.shipmentNo || shipmentId}.pdf`);
  ...
  const canReopen = shipment.status === 'DELIVERED';
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          {canReopen && (
            <Button variant="secondary" onClick={handleReopen} isLoading={reopenShipment.isPending}>
              <ReopenIcon />
              Reopen
            </Button>
          )}
