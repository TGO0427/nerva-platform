...
import {
  ...
  ImportShipmentLineData,
} from '@/lib/queries';
import { useDebounce } from '@/lib/hooks/use-debounce';
import { getVesselTrackingUrl } from '@nerva/shared';
...
export default function EditImportShipmentPage() {
  ...
  useEffect(() => {
    ...
    setLines(
      shipment.lines.map((line) => ({
        ...
        vesselOrAwb: line.vesselOrAwb || '',
        quantity: line.quantity != null ? String(line.quantity) : '',
        cbm: line.cbm != null ? String(line.cbm) : '',
        palletQty: line.palletQty != null ? String(line.palletQty) : '',
  ...
  const handleSubmit = async () => {
    ...
    if (lines.some((line) => !line.productDescription.trim())) {
      setError('Please enter a product description for every line');
      return;
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    />
                    {(() => {
                      const trackingUrl = getVesselTrackingUrl(line.transportMode, line.vesselOrAwb);
                      return trackingUrl ? (
                        <a
                          href={trackingUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          title={line.transportMode === 'AIR' ? 'Track AWB' : 'Track vessel on VesselFinder'}
                ...
                <div>
                  ...
                  <Input type="number" value={line.quantity} onChange={(e) => updateLine(index, { quantity: e.target.value })} min="0" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">CBM</label>
                  <Input type="number" value={line.cbm} onChange={(e) => updateLine(index, { cbm: e.target.value })} min="0" step="0.01" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Pallet Qty</label>
                  <Input type="number" value={line.palletQty} onChange={(e) => updateLine(index, { palletQty: e.target.value })} min="0" />
                </div>
                <div className="lg:col-span-3">
